    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          <div className="bg-white rounded-xl border p-4"><DollarSign className="w-5 h-5 text-green-600 mb-2" /><div className="text-xl font-bold">0 €</div><div className="text-xs text-gray-400">Выручка</div></div>
          <div className="bg-white rounded-xl border p-4"><ShoppingBag className="w-5 h-5 text-blue-600 mb-2" /><div className="text-xl font-bold">0</div><div className="text-xs text-gray-400">Заказы</div></div>
          <div className="bg-white rounded-xl border p-4"><Eye className="w-5 h-5 text-yellow-600 mb-2" /><div className="text-xl font-bold">{listings.reduce((sum, l) => sum + (l.views || 0), 0)}</div><div className="text-xs text-gray-400">Просмотры</div></div>
          <div className="bg-white rounded-xl border p-4"><Package className="w-5 h-5 text-purple-600 mb-2" /><div className="text-xl font-bold">{listings.length}</div><div className="text-xs text-gray-400">Товары</div></div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr className="text-left text-sm text-gray-500">
                    <th className="px-6 py-3">Название</th><th className="px-6 py-3">Цена</th><th className="px-6 py-3">Город</th><th className="px-6 py-3">Просмотры</th><th className="px-6 py-3"></th>
                  </table>
                </thead>
                <tbody className="divide-y">
                  {listings.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4"><button onClick={() => deleteListing(item.id)} className="text-red-500"><Trash2 className="w-4 h-4" /></button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
